...
echo ${name}
echo ${fname}
if [ -e ./STAR_output/${name}Log.final.out  ]; then
    echo "File exists, continuing"
    else
    ~/programs/STAR/source/STAR --genomeDir ${genomeDir} \
	    --runThreadN 15 \
	    --readFilesIn ${fastq} \
	    --outFileNamePrefix ./STAR_output/${name} \
	    --outSAMtype BAM SortedByCoordinate \
	    --quantMode GeneCounts \
	    --limitBAMsortRAM 80000000000
fi
if [ -e ./STAR_output/QC/${fname} ]; then
    echo "File exists, continuing"
    else
    java -Xmx24G -jar ~/programs/QoRTs-1.3.0/QoRTs.jar QC \
	    --maxReadLength 800 \
	    --stranded \
	    ...
	    --genomeFA ${GENOME} \
	    --rawfastq ${fastq} \
	    ./STAR_output/${name}Aligned.sortedByCoord.out.bam ${GTF} \
	    ./STAR_output/QC/${fname}
fi
done
...
if  [ -e ./STAR_output_all/Log.final.out ]; then
    echo "File exists, continuing"
    else
    ~/programs/STAR/source/STAR \
	    --outSAMattrRGline ${RGstring} \
	    --genomeDir ${genomeDir} \
	    --runThreadN 14 \
	    --readFilesIn ${star_input} \
	    --outFileNamePrefix ./STAR_output_all/ \
	    --outSAMtype BAM SortedByCoordinate \
...
Rscript="${HOME}/programs/R-3.4.0/bin/Rscript"
